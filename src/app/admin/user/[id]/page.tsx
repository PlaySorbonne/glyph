...
export default async function EditUserPage({ params }: { params: { id: string } }) {
  ...
  const updateUserAction = async (formData: FormData) => {
    ...
    const userData: Partial<UserInput> = {
      ...
      displayName: formData.get('displayName') ? formData.get('displayName') as string : user.displayName,
      email: formData.get('email') ? formData.get('email') as string : user.email,
      image: formData.get('image') ? formData.get('image') as string : user.image,
    };

    try {
      await updateUser(params.id, userData);
      revalidatePath(new URL("/admin/user/" + params.id, process.env.MAIN_URL).toString());
      revalidatePath(new URL("/admin/user/all", process.env.MAIN_URL).toString());
      revalidatePath(new URL("/", process.env.MAIN_URL).toString());
      redirect(new URL("/admin/user/all", process.env.MAIN_URL).toString());
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8 text-center">Modifier l'utilisateur</h1>
      <div className="bg-white shadow-md rounded-lg p-6">
        <form action={updateUserAction} className="space-y-4">
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-gray-700">Name</label>
            <input type="text" id="name" name="name" defaultValue={user.name ?? undefined}
                   className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" />
          </div>

          <div>
            <label htmlFor="displayName" className="block text-sm font-medium text-gray-700">Display Name</label>
            <input type="text" id="displayName" name="displayName" defaultValue={user.displayName ?? undefined}
                   className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" />
          </div>
